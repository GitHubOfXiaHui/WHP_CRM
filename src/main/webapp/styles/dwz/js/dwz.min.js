...
var DWZ = {keyCode:{ENTER:13, ESC:27, END:35, HOME:36, SHIFT:16, TAB:9, LEFT:37, RIGHT:39, UP:38, DOWN:40, DELETE:46, BACKSPACE:8}, eventType:{pageClear:"pageClear", resizeGrid:"resizeGrid"}, isOverAxis:function (x, reference, size) {
	return (x > reference) && (x < (reference + size));
}, isOver:function (y, x, top, left, height, width) {
	return this.isOverAxis(y, top, height) && this.isOverAxis(x, left, width);
}, pageInfo:{pageNum:"pageNum", numPerPage:"numPerPage", orderField:"orderField", orderDirection:"orderDirection"}, statusCode:{ok:200, error:300, timeout:301, forbidden:403}, ui:{sbar:true}, frag:{}, _msg:{}, _set:{loginUrl:"", loginTitle:"", debug:false}, msg:function (key, args) {
	...
	jQuery.ajax({type:"GET", url:pageFrag, dataType:"xml", timeout:50000, cache:false, error:function (xhr) {
...
(function ($) {
	...
	$.fn.extend({ajaxUrl:function (op) {
		var $this = $(this);
		$this.trigger(DWZ.eventType.pageClear);
		$.ajax({type:op.type || "GET", url:op.url, data:op.data, cache:false, success:function (response) {
		...
		}, error:DWZ.ajaxError, statusCode:{503:function (xhr, ajaxOptions, thrownError) {
		...
		return this.each(function () {
			...
			function altBoxCss(opacity) {
				...
				return {width:$this.width(), top:position.top + "px", left:position.left + "px", opacity:opacity || 1};
			}
			if (getAltBox().size() < 1) {
				if (!$this.attr("id")) {
...
(function ($) {
	$.fn.jDrag = function (options) {
		...
		return this.each(function () {
			var el = $(this);
			$.data($.rwdrag, "pp-rwdrag", {options:$.extend({el:el, obj:el}, options)});
			if (options.event) {
	...
	$.rwdrag = {start:function (e) {
		document.onselectstart = function (e) {
			return false;
		};
		var data = $.data(this, "pp-rwdrag");
		var el = data.options.el[0];
		$.data(el, "pp-rwdrag", {options:data.options});
		if (!$.rwdrag.current) {
			$.rwdrag.current = {el:el, oleft:parseInt(el.style.left) || 0, otop:parseInt(el.style.top) || 0, ox:e.pageX || e.screenX, oy:e.pageY || e.screenY};
...
(function ($) {
	$.extend($.fn, {jTree:function (options) {
		...
		return this.each(function () {
			...
			$(">li", $this).each(function () {
				...
				$li.genTree({icon:$this.hasClass("treeFolder"), ckbox:$this.hasClass("treeCheck"), options:op, level:0, exp:(cnum > 1 ? (first ? op.firstExp : (last ? op.lastExp : op.exp)) : op.endExp), coll:(cnum > 1 ? (first ? op.firstColl : (last ? op.lastColl : op.coll)) : op.endColl), showSub:(!$this.hasClass("collapse") && ($this.hasClass("expand") || (cnum > 1 ? (first ? true : false) : true))), isLast:(cnum > 1 ? (last ? true : false) : true)});
			});
			setTimeout(function () {
		...
		return this.each(function () {
			$(">li", this).each(function () {
				...
				$this.genTree({icon:op.icon, ckbox:op.ckbox, exp:isLast ? op.options.lastExp : op.options.exp, coll:isLast ? op.options.lastColl : op.options.coll, options:op.options, level:level, space:isLast ? null : op.space, showSub:op.showSub, isLast:isLast});
			});
		});
...
(function ($) {
	...
	$.dwz.accordion.prototype = {activate:function (index) {
	...
	function toggle(toShow, toHide, data, clickedActive, down) {
		...
		if (options.animated) {
			if (!options.alwaysOpen && clickedActive) {
				$.dwz.accordion.animations[options.animated]({toShow:jQuery([]), toHide:toHide, complete:complete, down:down, autoheight:options.autoheight});
			} else {
				$.dwz.accordion.animations[options.animated]({toShow:toShow, toHide:toHide, complete:complete, down:down, autoheight:options.autoheight});
			}
		} else {
			if (!options.alwaysOpen && clickedActive) {
				toShow.toggle();
	...
	$.extend($.dwz.accordion, {defaults:{selectedClass:"collapsable", alwaysOpen:true, animated:"slide", event:"click", header:".accordionHeader", autoheight:true, running:0, navigationFilter:function () {
...
function initUI(_box) {
	...
	if ($.fn.uploadify) {
		$(":file[uploaderOption]", $p).each(function () {
			var $this = $(this);
			var options = {fileObjName:$this.attr("name") || "file", auto:true, multi:true, onUploadError:uploadifyError};
	...
	$("form.required-validate", $p).each(function () {
		var $form = $(this);
		$form.validationEngine("attach", {scroll:false, ajaxFormValidation:true, promptPosition:"centerRight"});
	});
	if ($.fn.datepicker) {
	...
	$("div.pagination", $p).each(function () {
		var $this = $(this);
		$this.pagination({targetType:$this.attr("targetType"), rel:$this.attr("rel"), totalCount:$this.attr("totalCount"), numPerPage:$this.attr("numPerPage"), pageNumShown:$this.attr("pageNumShown"), currentPage:$this.attr("currentPage")});
	});
	if ($.fn.sortDrag) {
	...
	if ($.fn.dwzExportUrl) {
		$("a[target=dwzExportUrl]", $p).dwzExportUrl();
	}
	if ($.fn.lookup) {
...
function readyToolbarCSS() {
	...
	$a.each(function () {
		...
		var $span = $("span", this);
		if ($span.length == 0) {
			$(this).css({"background":"url(../styles/dwz/themes/css/images/toolbar_icons16/" + imageName + ".png) no-repeat", "background-position":"50% 50%", "width":"22px", "height":"20px", "text-indent":"-1000px", "overflow":"hidden", "display":"block", "float":"left"});
		} else {
			$span.css({"background-image":"url(../styles/dwz/themes/css/images/toolbar_icons16/" + imageName + ".png)", "background-position":"0 3px"});
		}
	});
}
(function ($) {
	$.fn.extend({theme:function (options) {
...
$.setRegional("alertMsg", {title:{error:"Error", info:"Information", warn:"Warning", correct:"Successful", confirm:"Confirmation"}, butMsg:{ok:"OK", yes:"Yes", no:"No", cancel:"Cancel"}});
var alertMsg = {_boxId:"#alertMsgBox", _bgId:"#alertBackground", _closeTimer:null, _types:{error:"error", info:"info", warn:"warn", correct:"correct", confirm:"confirm"}, _getTitle:function (key) {
...
(function ($) {
	...
	$.fn.extend({contextMenu:function (id, options) {
		var op = $.extend({shadow:true, bindings:{}, ctrSub:null}, options);
		if (!menu) {
		...
		hash = hash || [];
		hash.push({id:id, shadow:op.shadow, bindings:op.bindings || {}, ctrSub:op.ctrSub});
...
var navTab = {componentBox:null, _tabBox:null, _prevBut:null, _nextBut:null, _panelBox:null, _moreBut:null, _moreBox:null, _currentIndex:0, _op:{id:"navTab", stTabBox:".navTab-tab", stPanelBox:".navTab-panel", mainTabId:"main", close$:"a.close", prevClass:"tabsLeft", nextClass:"tabsRight", stMore:".tabsMore", stMoreLi:"ul.tabsMoreList"}, init:function (options) {
	if ($.History) {
	...
	var $this = this;
	$obj.contextMenu("navTabCM", {bindings:{reload:function (t, m) {
		$this._reload(t, true);
	...
	if ($panel) {
		...
		if (url) {
			if ($tab.hasClass("external")) {
				...
				$panel.ajaxUrl({type:"POST", url:url, data:op.data, callback:function (response) {
					navTab._loadUrlCallback($panel);
					if ($.isFunction(op.callback)) {
						op.callback(response);
					}
	...
	if (iOpenIndex >= 0) {
		...
		if (op.fresh || $tab.attr("url") != url) {
			...
			if (op.external || url.isExternalUrl()) {
				...
				$panel.ajaxUrl({type:"GET", url:url, data:op.data, callback:function () {
					navTab._loadUrlCallback($panel);
				}});
			}
		}
...
(function ($) {
	...
	$.resizeTool = {start:function (resizable, dialog, e, target) {
		$.pdialog.initResize(resizable, dialog, target);
		$.data(resizable[0], "layer-drag", {options:$.extend($.pdialog._op, {target:target, dialog:dialog, stop:$.resizeTool.stop})});
	...
	$.layerdrag = {start:function (obj, e, options) {
		if (!$.layerdrag.current) {
			$.layerdrag.current = {el:obj, oleft:parseInt(obj.style.left) || 0, owidth:parseInt(obj.style.width) || 0, otop:parseInt(obj.style.top) || 0, oheight:parseInt(obj.style.height) || 0, ox:e.pageX || e.screenX, oy:e.pageY || e.clientY};
...
(function ($) {
	$.pdialog = {_op:{height:300, width:580, minH:40, minW:50, total:20, max:false, mask:false, resizable:true, drawable:true, maxable:true, minable:true, fresh:true}, _current:null, _zIndex:42, getCurrent:function () {
		...
		if (dialog) {
			...
			jDContent.ajaxUrl({type:"POST", url:url, data:op.data, callback:function (response) {
		...
		shadow.css({top:parseInt($(dialog)[0].style.top) - 2, left:parseInt($(dialog)[0].style.left) - 4, height:parseInt($(dialog).height()) + 8, width:parseInt($(dialog).width()) + 8, zIndex:parseInt($(dialog).css("zIndex")) - 1});
		...
		dialog.css({left:($(window).width() - dialog.width()) / 2, top:iTop > 0 ? iTop : 0});
	}, initResize:function (resizable, dialog, target) {
		$("body").css("cursor", target + "-resize");
		resizable.css({top:$(dialog).css("top"), left:$(dialog).css("left"), height:$(dialog).css("height"), width:$(dialog).css("width")});
		resizable.show();
		...
		$(dialog).data("original", {top:$(dialog).css("top"), left:$(dialog).css("left"), width:$(dialog).css("width"), height:$(dialog).css("height")});
		$("a.maximize", dialog).hide();
		...
		$(dialog).css({top:original.top, left:original.left, width:dwidth, height:dheight});
		...
		$(".resizable").css({top:$(dialog).css("top"), left:$(dialog).css("left"), height:$(dialog).css("height"), width:$(dialog).css("width")}).show().animate({top:$(window).height() - 60, left:task.position().left, width:task.outerWidth(), height:task.outerHeight()}, 250, function () {
			$(this).hide();
...
(function ($) {
	...
	$.dialogDrag = {currId:null, _init:function (dialog) {
		...
		this._init(dialog);
		var sh = $("#dialogProxy");
		sh.css({left:dialog.css("left"), top:dialog.css("top"), height:dialog.css("height"), width:dialog.css("width"), zIndex:parseInt(dialog.css("zIndex")) + 1}).show();
		...
		$(".shadow").hide();
		$(sh).jDrag({selector:".dialogHeader", stop:this.stop, event:event});
		return false;
	}, stop:function () {
		var sh = $(arguments[0]);
...
(function ($) {
	var _op = {cursor:"move", sortBoxs:"div.sortDrag", replace:false, items:"> *", selector:"", zIndex:1000};
	var sortDrag = {start:function ($sortBox, $item, event, op) {
		...
		$helper.addClass("sortDragHelper").css({position:"absolute", top:position.top + $sortBox.scrollTop(), left:position.left, zIndex:op.zIndex, width:$item.width() + "px", height:$item.height() + "px"}).jDrag({selector:op.selector, drag:this.drag, stop:this.stop, event:event});
		...
		$helper.animate({top:(position.top + $sortBox.scrollTop()) + "px", left:position.left + "px"}, {complete:function () {
		...
		return $("<" + $item[0].nodeName + " class=\"sortDragPlaceholder\"/>").css({width:$item.outerWidth() + "px", height:$item.outerHeight() + "px", marginTop:$item.css("marginTop"), marginRight:$item.css("marginRight"), marginBottom:$item.css("marginBottom"), marginLeft:$item.css("marginLeft")});
...
(function ($) {
	$.fn.extend({cssTable:function (options) {
		return this.each(function () {
			var $this = $(this);
			...
			$this.find("thead [orderField]").orderBy({targetType:$this.attr("targetType"), rel:$this.attr("rel"), asc:$this.attr("asc") || "asc", desc:$this.attr("desc") || "desc"});
		});
	}});
})(jQuery);
(function ($) {
	$.fn.jTable = function (options) {
		return this.each(function () {
			...
			}).filter("[orderField]").orderBy({targetType:$table.attr("targetType"), rel:$table.attr("rel"), asc:$table.attr("asc") || "asc", desc:$table.attr("desc") || "desc"});
					...
					$th.mouseover(function (event) {
						...
						if ($th.outerWidth() - offset < 5) {
							$th.css("cursor", "col-resize").mousedown(function (event) {
								$(".resizeProxy", $grid).show().css({left:$.jTableTool.getRight(th) - $(".gridScroller", $grid).scrollLeft(), top:$.jTableTool.getTop(th), height:$.jTableTool.getHeight(th, $grid), cursor:"col-resize"});
								$(".resizeMarker", $grid).show().css({left:$.jTableTool.getLeft(th) + 1 - $(".gridScroller", $grid).scrollLeft(), top:$.jTableTool.getTop(th), height:$.jTableTool.getHeight(th, $grid)});
								$(".resizeProxy", $grid).jDrag($.extend(options, {scop:true, cellMinW:20, relObj:$(".resizeMarker", $grid)[0], move:"horizontal", event:event, stop:function () {
	...
	$.jTableTool = {getLeft:function (obj) {
		var width = 0;
		...
		if ($.browser.msie) {
			...
			var evtset = {offsetX:evt.pageX || evt.screenX, offsetY:evt.pageY || evt.screenY};
			var offset = {offsetX:evtset.offsetX - objset.left, offsetY:evtset.offsetY - objset.top};
			return offset;
		}
...
(function ($) {
	...
	$.taskBar = {_taskBar:null, _taskBox:null, _prevBut:null, _nextBut:null, _op:{id:"taskbar", taskBox:"div.taskbarContent", prevBut:".taskbarLeft", prevDis:"taskbarLeftDisabled", nextBut:".taskbarRight", nextDis:"taskbarRightDisabled", selected:"selected", boxMargin:"taskbarMargin"}, init:function (options) {
		var $this = this;
		$.extend(this._op, options);
		...
		$(obj).contextMenu("dialogCM", {bindings:{closeCurrent:function (t, m) {
		...
		if (this._taskBar.is(":hidden")) {
			...
			this._taskBar.animate({top:$(window).height() - this._taskBar.outerHeight()}, 500);
		}
	}, hide:function () {
		this._taskBar.animate({top:$(window).height() - 29 + this._taskBar.outerHeight(true)}, 500, function () {
...
function validateCallback(form, callback, confirmMsg) {
	...
	var _submitFn = function () {
		$.ajax({type:form.method || "POST", url:$form.attr("action"), data:$form.serializeArray(), dataType:"json", cache:false, success:callback || DWZ.ajaxDone, error:DWZ.ajaxError});
	};
	if (confirmMsg) {
		...
		if ("closeCurrent" == json.callbackType) {
			...
			if ("forward" == json.callbackType) {
				...
				if ("forwardConfirm" == json.callbackType) {
					alertMsg.confirm(json.confirmMsg || DWZ.msg("forwardConfirmMsg"), {okCall:function () {
						navTab.reload(json.forwardUrl);
					}, cancelCall:function () {
						navTab.closeCurrentTab(json.navTabId);
					}});
...
function divSearch(form, rel) {
	...
	if (rel) {
		var $box = $("#" + rel);
		$box.ajaxUrl({type:"POST", url:$form.attr("action"), data:$form.serializeArray(), callback:function () {
...
function dwzPageBreak(options) {
	...
	if (op.rel) {
		...
		if (form) {
			$box.ajaxUrl({type:"POST", url:$(form).attr("action"), data:params, callback:function () {
				$box.find("[layoutH]").layoutH();
			}});
		}
	} else {
...
function ajaxTodo(url, callback) {
	...
	$.ajax({type:"POST", url:url, dataType:"json", cache:false, success:$callback, error:DWZ.ajaxError});
}
...
},	dwzExportUrl: function(){
		function _doExportUrl($this,url) {
			var $p = $this.attr("targetType") == "dialog" ? $.pdialog.getCurrent() : navTab.getCurrentPanel();
			var $form = $("#pagerForm", $p);
			window.location = url+(url.indexOf('?') == -1 ? "?" : "&")+$form.serialize();
		}
			...
			$this.click(function(event){
				...
				if (title) {
					alertMsg.confirm(title, {
						okCall: function(){_doExportUrl($this);}
					});
				} else {
...
(function ($) {
	$.fn.extend({pagination:function (opts) {
		var setting = {first$:"li.j-first", prev$:"li.j-prev", next$:"li.j-next", last$:"li.j-last", nums$:"li.j-num>a", jumpto$:"li.jumpto", pageNumFrag:"<li class=\"#liClass#\"><a href=\"javascript:;\">#pageNum#</a></li>"};
		return this.each(function () {
			var $this = $(this);
	...
	var Pagination = function (opts) {
		this.opts = $.extend({targetType:"navTab", rel:"", totalCount:0, numPerPage:10, pageNumShown:10, currentPage:1, callback:function () {
			return false;
		}}, opts);
	};
	$.extend(Pagination.prototype, {targetType:function () {
...
(function ($) {
	...
	var _util = {_lookupPrefix:function (key) {
	...
	$.extend({bringBackSuggest:function (args) {
		...
		$box.find(":input").each(function () {
			var $input = $(this), inputName = $input.attr("val") || $input.attr("name");
			for (var key in args) {
	...
	$.fn.extend({lookup:function () {
		return this.each(function () {
			var $this = $(this), options = {mask:true, width:$this.attr("width") || 820, height:$this.attr("height") || 400, maxable:eval($this.attr("maxable") || "true"), resizable:eval($this.attr("resizable") || "true")};
			$this.click(function (event) {
				_lookup = $.extend(_lookup, {currentGroup:$this.attr("lookupGroup") || "", suffix:$this.attr("suffix") || "", $target:$this, pk:$this.attr("lookupPk") || "id"});
				var url = unescape($this.attr("href")).replaceTmById($(event.target).parents(".unitBox:first"));
				if (!url.isFinishedTm()) {
					alertMsg.error($this.attr("warn") || DWZ.msg("alertSelectMsg"));
					return false;
		...
		return this.each(function () {
			...
			function _show(event) {
				...
				$suggest.css({left:offset.left + "px", top:iTop + "px"}).show();
				_lookup = $.extend(_lookup, {currentGroup:$input.attr("lookupGroup") || "", suffix:$input.attr("suffix") || "", $target:$input, pk:$input.attr("lookupPk") || "id"});
				...
				$.ajax({global:false, type:"POST", dataType:"json", url:url, cache:false, data:postData, success:function (response) {
					if (!response) {
						return;
					}
					var html = "";
		...
		return this.each(function () {
			...
			$tbody.find("a.btnDel").click(function () {
				...
				function delDbData() {
					$.ajax({type:"POST", dataType:"json", url:$btnDel.attr("href"), cache:false, success:function () {
						$btnDel.parents("tr:first").remove();
						initSuffix($tbody);
					}, error:DWZ.ajaxError});
				}
		...
		function tdHtml(field) {
			...
			switch (field.type) {
			  ...
			  case "lookup":
				...
				html = "<input type=\"hidden\" name=\"" + field.lookupGroup + "." + field.lookupPk + suffix + "\"/>" + "<input type=\"text\" name=\"" + field.name + "\"" + suggestFrag + " lookupPk=\"" + field.lookupPk + "\" size=\"" + field.size + "\" class=\"" + field.fieldClass + "\" readonly=\"readonly\"/>" + "<a class=\"btnLook\" href=\"" + field.lookupUrl + "\" lookupGroup=\"" + field.lookupGroup + "\" " + suggestFrag + " lookupPk=\"" + field.lookupPk + "\" title=\"\u67e5\u627e\u5e26\u56de\">\u67e5\u627e\u5e26\u56de</a>";
				break;
			  case "attach":
				...
				break;
			  case "enum":
				$.ajax({type:"POST", dataType:"html", async:false, url:field.enumUrl, data:{inputName:field.name}, success:function (response) {
					html = response;
				}});
				break;
			  case "date":
				html = "<input type=\"text\" name=\"" + field.name + "\" value=\"" + field.defaultVal + "\" class=\"date " + field.fieldClass + "\" dateFmt=\"" + field.patternDate + "\" size=\"" + field.size + "\"/>" + "<a class=\"\" href=\"javascript:void(0)\"></a>";
				break;
			  case "texthid":
				html = "<input type=\"text\" name=\"" + field.name + "\" value=\"" + field.defaultVal + "\" size=\"" + field.size + "\" readonly=\"readonly\"/>";
				break;
			  default:
		...
		return this.each(function () {
			...
			$this.click(function () {
				...
				function _doPost() {
					$.ajax({type:"POST", url:$this.attr("href"), dataType:"json", cache:false, data:function () {
						if (postType == "map") {
...
(function ($) {
	...
	$.fn.datepicker = function (opts) {
		...
		return this.each(function () {
			...
			$this.click(function (event) {
				...
				var iTop = offset.top + this.offsetHeight;
				$(DWZ.frag["calendarFrag"]).appendTo("body").css({left:offset.left + "px", top:iTop + "px"}).show().click(function (event) {
					event.stopPropagation();
				});
	...
	var Datepicker = function (sDate, opts) {
		this.opts = $.extend({pattern:"yyyy-MM-dd", minDate:"1900-01-01", maxDate:"2099-12-31", mmStep:1, ssStep:1}, opts);
		var now = new Date();
	...
	$.extend(Datepicker.prototype, {get:function (name) {
		...
		return {year:y, month:m, day:date.getDate(), hour:date.getHours(), minute:date.getMinutes(), second:date.getSeconds(), days:days, date:date};
...
(function ($) {
	$.extend($.fn, {jBlindUp:function (options) {
		var op = $.extend({duration:500, easing:"swing", call:function () {
		}}, options);
		return this.each(function () {
			var $this = $(this);
			$(this).animate({height:0}, {step:function () {
		...
		return this.each(function () {
			...
			$this.animate({height:fixedPanelHeight}, {step:function () {
			}, duration:op.duration, easing:op.easing, complete:function () {
				$this.css({display:""});
				op.call();
			}});
		...
		return this.each(function () {
			...
			$(">div", $this).css({position:"relative"}).animate({top:-fixedHeight}, {easing:op.easing, duration:op.duration, complete:function () {
				$this.html($(this).html());
			}});
		});
		...
		return this.each(function () {
			...
			$this.css({overflow:"visible", position:"relative", height:"0px"}).animate({height:fixedHeight}, {duration:op.duration, easing:op.easing, complete:function () {
				$this.css({display:"", overflow:""});
				op.call();
			}});
			$(">div", $this).css({position:"relative"}).animate({top:0}, {easing:op.easing, duration:op.duration, complete:function () {
				$this.html($(this).html());
			}});
		});
	}});
	$.effect = {getDimensions:function (element, displayElement) {
...
(function ($) {
	$.extend($.fn, {jPanel:function (options) {
...
(function ($) {
	$.fn.extend({checkboxCtrl:function (parent) {
		return this.each(function () {
			var $trigger = $(this);
	...
	$.checkbox = {selectAll:function (_name, _parent) {
...
(function ($) {
	...
	$.extend($.fn, {comboxSelect:function (options) {
		...
		return this.each(function (i) {
			...
			if (ref && refUrl) {
				function _onchange(event) {
					...
					$.ajax({type:"POST", dataType:"json", url:refUrl.replace("{value}", encodeURIComponent($this.attr("value"))), cache:false, data:{}, success:function (json) {
						if (!json) {
							return;
						}
						var html = "";
...
(function ($) {
	$.extend({History:{_hash:new Array(), _cont:undefined, _currentHash:"", _callback:undefined, init:function (cont, callback) {
		$.History._cont = cont;
