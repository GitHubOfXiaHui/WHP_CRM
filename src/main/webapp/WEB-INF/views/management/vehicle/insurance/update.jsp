...
<form method="post" id="insuranceCreateForm"
	action="${contextPath }/management/vehicle/insurance/update"
	class="required-validate pageForm"
	onsubmit="return validateCallback(this, dialogReloadNavTab);">
	<div class="pageFormContent" layoutH="97">
		<p>
			<label>车牌号：</label> <input type="text" value='${vehicle.license }'
				readonly="readonly" /> <input type="hidden" name="id"
				value='${vehicle.id }' />
		</p>
		<p>
			<label>车辆型号：</label> <input type="text" value='${vehicle.type }'
				readonly="readonly" />
		</p>
		<p>
			<label>车辆配置：</label> <input type="text"
				value='${vehicle.configuration }' readonly="readonly" />
		</p>
		<p>
			<label>排量：</label> <input type="text"
				value='${vehicle.displacement }' readonly="readonly" />
		</p>
		<p>
			<label>购买时间：</label> <input type="text"
				value='<fmt:formatDate value="${vehicle.buyingTime}" pattern="yyyy-MM-dd"/>'
				readonly="readonly" />
		</p>
					...
					<table class="list nowrap itemDetail" addButton="添加" width="100%">
						<thead align="center">
							<tr>
								...
								<th type="text" name="insuranceList[#index#].insuranceCarriers"
									size="20" fieldClass="validate[required,maxSize[64]] required">承保公司</th>
								<th type="text" name="insuranceList[#index#].insuranceCost"
									fieldClass="validate[required,maxSize[10]] required" size="20">保险价格（元）</th>
								<th type="text" name="insuranceList[#index#].insuranceAmount"
									fieldClass="validate[maxSize[10]]" size="20" defaultVal="0">保险额度（万元）
								</th>
								...
								<th type="text" name="insuranceList[#index#].insuranceRemark"
									size="30">备注</th>
						...
						<tbody align="center">
							<c:forEach var="child" items="${vehicle.insuranceList}"
								varStatus="status">
								<tr class="unitBox">
									<input type="hidden" name="linkmanList[${status.index}].id"
										value="${child.id}" />
									<input type="hidden"
										name="insuranceList[${status.index}].parent.id"
										value="${child.parent.id}" />
									<td><select name="insuranceList[${status.index}].insuranceType"
									...
									<td><input type="text" size="12" datefmt="yyyy-MM-dd" class="date required" size="10" value="<fmt:formatDate value="${child.insuredTime}" pattern="yyyy-MM-dd"/>"  name="insuranceList[${status.index}].insuredTime"/></td>
									<td><input type="text" size="12" datefmt="yyyy-MM-dd" class="date required" size="10" value="<fmt:formatDate value="${child.expireTime}" pattern="yyyy-MM-dd"/>"  name="insuranceList[${status.index}].expireTime"/></td>
									<td><input type="text" name="insuranceList[${status.index}].insuranceRemark" value="${child.insuranceRemark}"/></td>
