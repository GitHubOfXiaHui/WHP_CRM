...
<form method="post" action="${contextPath}/management/security/user/update" class="required-validate pageForm" onsubmit="return validateCallback(this, dialogReloadNavTab);">
	...
	<div class="pageFormContent" layoutH="58">
		...
		<p>
			...
			<label>用户级别：</label>
			<select name="userb">
				<option value="1" ${user.status == "1" ? 'selected="selected"' : ''}>业务操作员</option>
				<option value="2" ${user.status == "2" ? 'selected="selected"' : ''}>业务管理员</option>
			</select>
		</p>
		<p>
			<label>关联组织：</label>
			<input name="organization.id" val="organization.id" value="${user.organization.id }" type="hidden"/>
			<input class="validate[required] required" name="organization.name" val="organization.name" type="text" readonly="readonly" value="${user.organization.name }"/>
