...
<keta:paginationForm action="${contextPath }/management/security/user/list" page="${page }">
	<input type="hidden" name="search_LIKE_username" value="${LIKE_username }"/>
</keta:paginationForm>
	...
	<div class="pageHeader">
		<div class="searchBar">
			<ul class="searchContent">
				<li>
					<label>登录名称：</label>
					<input type="text" name="search_LIKE_username" value="${LIKE_username }"/>
				</li>
			</ul>
			<div class="subBar">
				<ul>						
	...
	<div class="panelBar">
		<ul class="toolBar">
			<shiro:hasPermission name="User:save">
				<li><a iconClass="user_add" target="dialog" rel="lookup2organization_add" mask="true" width="630" height="350" href="${contextPath }/management/security/user/create"><span>添加用户</span></a></li>
			</shiro:hasPermission>
	...
	<table class="table" layoutH="137" width="100%">
		...
		<tbody>
			...
			<tr target="slt_uid" rel="${item.id}">
			   ...
			   <td>
			   <c:if test="${item.userb=='1'}"> 
			          业务操作员
			   </c:if>
			   <c:if test="${item.userb=='2'}"> 
			          业务管理员
			   </c:if>
			   </td>
